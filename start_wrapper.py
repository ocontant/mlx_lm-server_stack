...
class DependencyManager:
    ...
    def check_pyenv_installed(self) -> bool:
        ...
        try:
            ...
            self.logger.debug("pyenv command not found or not working")
            return False
        except Exception as e:
            self.logger.debug(f"Unexpected error checking pyenv: {e}")
            return False
...
class ServiceManager:
    ...
    def start_mlx_service(self) -> subprocess.Popen:
        ...
        """
        # Explicitly set up pyenv context
        venv_name = self.args.venv_name
        pyenv_root = os.environ.get("PYENV_ROOT", os.path.expanduser("~/.pyenv"))

        # Set required pyenv environment variables
        env = os.environ.copy()
        env["PYENV_VERSION"] = venv_name
        env["PYENV_ROOT"] = pyenv_root
        env["VIRTUAL_ENV"] = os.path.join(pyenv_root, "versions", venv_name)
        env["PATH"] = f"{env['VIRTUAL_ENV']}/bin:{pyenv_root}/bin:{env['PATH']}"

        # Log pyenv context for debugging
        self.logger.debug("PyEnv Context:")
        self.logger.debug(f"  PYENV_VERSION: {env['PYENV_VERSION']}")
        self.logger.debug(f"  PYENV_ROOT: {env['PYENV_ROOT']}")
        self.logger.debug(f"  VIRTUAL_ENV: {env['VIRTUAL_ENV']}")
        self.logger.debug(f"  PATH: {env['PATH']}")
        ...
        self.logger.debug("MLX Service Environment Variables:")
        for key, value in env.items():
            if key.startswith("MLX_") or key in [
                "LOG_LEVEL",
                "TRUST_REMOTE_CODE",
                "EXTRA_ARGS",
            ]:
        ...
        # Get the mlx_lm.server path using pyenv
        try:
            result = subprocess.run(
                "pyenv which mlx_lm.server",
                shell=True,
                check=True,
                capture_output=True,
                text=True,
            ...
            mlx_server_path = result.stdout.strip()
            self.logger.debug(f"Found mlx_lm.server at: {mlx_server_path}")
        except subprocess.CalledProcessError as e:
            self.logger.error(f"Failed to locate mlx_lm.server: {e}")
            self.logger.debug(f"Command output: {e.output}")
            # Fallback to direct path construction
            mlx_server_path = os.path.join(env["VIRTUAL_ENV"], "bin/mlx_lm.server")
            self.logger.debug(f"Falling back to constructed path: {mlx_server_path}")

        if not os.path.exists(mlx_server_path):
            self.logger.error(
                f"MLX server executable not found at {mlx_server_path}. Please check your installation."
            )
            sys.exit(1)

        try:
            # Construct command as a list for better argument handling
            cmd = [
                mlx_server_path,
                "--model",
                env["MLX_MODEL"],
            ...
            if env["TRUST_REMOTE_CODE"].lower() == "true":
                cmd.append("--trust-remote-code")

            if env["EXTRA_ARGS"]:
                cmd.extend(env["EXTRA_ARGS"].split())
            ...
            # Start threads to continuously process both stdout and stderr
            import threading
            ...
            def log_output(stream, prefix):
                try:
                    for line in stream:
                        line = line.strip()
                        if line:
                            # Performance metrics should be INFO level
                            if any(
                                metric in line.lower()
                                for metric in ["Prompt:", "Peak memory", "Generation"]
                            ):
                                self.logger.info(f"{prefix} {line}")
                            # Other messages based on their content
                    ...
                    self.logger.error(f"Error in output processing thread: {e}")

            # Start separate threads for stdout and stderr
            threading.Thread(
                target=log_output,
                args=(process.stdout, "[MLX]"),
                daemon=True,
                name="MLXStdoutThread",
            ).start()

            threading.Thread(
                target=log_output,
                args=(process.stderr, "[MLX ERR]"),
                daemon=True,
                name="MLXStderrThread",
            ).start()

            # Check immediate startup issues
            time.sleep(1)
            if process.poll() is not None:
                self.logger.error(
                    f"MLX service failed to start (exit code: {process.poll()})"
                )
                out, err = process.communicate()
                if out:
                    self.logger.error(f"Last stdout: {out.strip()}")
                if err:
                    self.logger.error(f"Last stderr: {err.strip()}")
                raise RuntimeError("MLX service failed to start")
    ...
    def run_services(self) -> None:
        ...
        try:
            ...
            # Check if the MLX process terminated immediately
            if self.mlx_process.poll() is not None:
                exit_code = self.mlx_process.poll()
                self.logger.error(
                    f"MLX service exited immediately with code {exit_code}"
                )
                # Try to get error output
                output, _ = self.mlx_process.communicate()
                self.logger.error(f"MLX service output: {output}")
                return
            ...
            # Keep the main thread running instead of waiting for process completion
            while self.mlx_process.poll() is None:
                time.sleep(1)  # Check every second

            # If we get here, MLX process has terminated unexpectedly
            exit_code = self.mlx_process.poll()
            self.logger.warning(
                f"MLX service exited unexpectedly with code {exit_code}"
            )
...
def parse_arguments() -> argparse.Namespace:
    ...
    # mlx_lm-server doesn't support a EOS_Token argument yet
    # service_group.add_argument(
    #     "--eos-token",
    #     default="",
...
## EOS Token is not supported yet
# EOS_TOKEN="<|endoftext|>"
EXTRA_ARGS=
SKIP_DOCKER=false
"""
